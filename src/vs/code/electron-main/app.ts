...
import { IEtheranaUpdateService } from '../../workbench/contrib/etherana/common/etheranaUpdateService.js';
import { MetricsMainService } from '../../workbench/contrib/etherana/electron-main/metricsMainService.js';
import { EtheranaMainUpdateService } from '../../workbench/contrib/etherana/electron-main/etheranaUpdateMainService.js';
import { LLMMessageChannel } from '../../workbench/contrib/etherana/electron-main/sendLLMMessageChannel.js';
import { EtheranaSCMService } from '../../workbench/contrib/etherana/electron-main/etheranaSCMMainService.js';
import { IEtheranaSCMService } from '../../workbench/contrib/etherana/common/etheranaSCMTypes.js';
import { MCPChannel } from '../../workbench/contrib/etherana/electron-main/mcpChannel.js';
/**
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Etherana main process services (required for services with a channel for comm between browser and electron-main (node))
		services.set(IMetricsService, new SyncDescriptor(MetricsMainService, undefined, false));
		services.set(IEtheranaUpdateService, new SyncDescriptor(EtheranaMainUpdateService, undefined, false));
		services.set(IEtheranaSCMService, new SyncDescriptor(EtheranaSCMService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Etherana - use loggerChannel as reference
		const metricsChannel = ProxyChannel.fromService(accessor.get(IMetricsService), disposables);
		mainProcessElectronServer.registerChannel('etherana-channel-metrics', metricsChannel);

		const etheranaUpdatesChannel = ProxyChannel.fromService(accessor.get(IEtheranaUpdateService), disposables);
		mainProcessElectronServer.registerChannel('etherana-channel-update', etheranaUpdatesChannel);

		const sendLLMMessageChannel = new LLMMessageChannel(accessor.get(IMetricsService));
		mainProcessElectronServer.registerChannel('etherana-channel-llmMessage', sendLLMMessageChannel);

		// Etherana added this
		const etheranaSCMChannel = ProxyChannel.fromService(accessor.get(IEtheranaSCMService), disposables);
		mainProcessElectronServer.registerChannel('etherana-channel-scm', etheranaSCMChannel);

		// Etherana added this
		const mcpChannel = new MCPChannel();
		mainProcessElectronServer.registerChannel('etherana-channel-mcp', mcpChannel);

...
export class UserDataSyncWorkbenchContribution extends Disposable implements IWorkbenchContribution {
		...
		switch (error.code) {
			...
			case UserDataSyncErrorCode.DefaultServiceChanged:
				...
				if (this.userDataSyncEnablementService.isEnabled()) {
					this.notificationService.notify({
						severity: Severity.Info,
						message: localize('using separate service', "Settings sync now uses a separate service, more information is available in the [Settings Sync Documentation](https://goanon.pro#_syncing-stable-versus-insiders)."),
					});
				}
	...
	private registerHelpAction(): void {
		...
		this._register(registerAction2(class HelpAction extends Action2 {
			...
			run(): unknown { return that.openerService.open(URI.parse('https://goanon.pro')); }
		}));

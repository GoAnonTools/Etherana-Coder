...
import { displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, ProviderName, SettingsOfProvider } from '../../common/etheranaSettingsTypes.js';
...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	else throw new Error(`Etherana providerName was invalid: ${providerName}.`)
}
...
const openAITools = (safetyMode: SafetyMode | null, mcpTools: InternalToolInfo[] | undefined) => {
	const allowedTools = availableTools(safetyMode, mcpTools)
...
const _sendOpenAICompatibleChat = async ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, modelName: modelName_, _setAborter, providerName, safetyMode, separateSystemMessage, overridesOfModel, mcpTools }: SendChatParams_Internal) => {
	const {
		modelName,
		specialToolFormat,
	...
	// tools
	const potentialTools = openAITools(safetyMode, mcpTools)
	...
	if (!specialToolFormat) {
		const { newOnText, newOnFinalMessage } = extractXMLToolsWrapper(onText, onFinalMessage, safetyMode, mcpTools)
		onText = newOnText
		onFinalMessage = newOnFinalMessage
	}
...
const anthropicTools = (safetyMode: SafetyMode | null, mcpTools: InternalToolInfo[] | undefined) => {
	const allowedTools = availableTools(safetyMode, mcpTools)
	if (!allowedTools || Object.keys(allowedTools).length === 0) return null
...
// ------------ ANTHROPIC ------------
const sendAnthropicChat = async ({ messages, providerName, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, overridesOfModel, modelName: modelName_, _setAborter, separateSystemMessage, safetyMode, mcpTools }: SendChatParams_Internal) => {
	const {
		modelName,
		specialToolFormat,
	} = getModelCapabilities(providerName, modelName_, overridesOfModel)
	...
	// tools
	const potentialTools = anthropicTools(safetyMode, mcpTools)
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint) throw new Error(`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in Etherana if you want the default url).`)
...
const geminiTools = (safetyMode: SafetyMode | null, mcpTools: InternalToolInfo[] | undefined): GeminiTool[] | null => {
	const allowedTools = availableTools(safetyMode, mcpTools)
	if (!allowedTools || Object.keys(allowedTools).length === 0) return null
...
const sendGeminiChat = async ({
	...
	// tools
	const potentialTools = geminiTools(safetyMode, mcpTools)

...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/Etherana
	...
	private get oldId() {
		...
		const newKey = 'etherana.app.oldMachineId'
		...
		const oldValue = this._appStorage.get('etherana.machineId', StorageScope.APPLICATION) ?? 'NULL' // the old way of getting the key
		...
		// private get oldId() {
		// 	return this._memoStorage('etherana.app.oldMachineId', StorageTarget.MACHINE, 'NULL')
		// }
	}
	...
	private get distinctId() {
		...
		return this._memoStorage('etherana.app.machineId', StorageTarget.MACHINE, setValIfNotExist)
	}

	// just to see if there are ever multiple machineIDs per userID (instead of this, we should just track by the user's email)
	private get userId() {
		return this._memoStorage('etherana.app.userMachineId', StorageTarget.USER)
	}
	...
	async initialize() {
		...
		console.log('User is opted out of basic Etherana metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('Etherana posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}

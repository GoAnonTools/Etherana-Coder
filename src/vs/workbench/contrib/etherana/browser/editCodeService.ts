...
import { voidPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, diffRationale_systemMessage, diffRationale_userMessage } from '../common/prompt/prompts.js';
import { IEtheranaCommandBarService } from './etheranaCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { ETHERANA_ACCEPT_DIFF_ACTION_ID, ETHERANA_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { DiffArea, Diff, CtrlKZone, EtheranaFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._etheranaModelService.initializeModel(model.uri)
	...
	// 		message: `Etherana Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'etherana.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Etherana Coder's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(ETHERANA_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/etherana/etherana/issues/new) it.` : undefined
	// 	})
	// }
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'etherana-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'etherana-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'etherana-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'etherana-greenBG', {
				...
				hoverMessage: diff.rationale ? { value: `**Rationale:** ${diff.rationale}`, isTrusted: true } : undefined
			})
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'etherana-redBG'
		...
		if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
			...
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					else { throw new Error('Etherana 1') }
	...
	private _getCurrentEtheranaFileSnapshot = (uri: URI): EtheranaFileSnapshot => {
		const { model } = this._etheranaModelService.getModel(uri)
	...
	private _restoreEtheranaFileSnapshot = async (uri: URI, snapshot: EtheranaFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: EtheranaFileSnapshot = this._getCurrentEtheranaFileSnapshot(uri)
		let afterSnapshot: EtheranaFileSnapshot | null = null

		const elt: IUndoRedoElement = {
			type: UndoRedoElementType.Resource,
			resource: uri,
			label: 'Etherana Agent',
			code: 'undoredo.editCode',
			undo: async () => { opts?.onWillUndo?.(); await this._restoreEtheranaFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreEtheranaFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentEtheranaFileSnapshot(uri)
			await this._etheranaModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getEtheranaFileSnapshot(uri: URI) {
		return this._getCurrentEtheranaFileSnapshot(uri)
	}


	public restoreEtheranaFileSnapshot(uri: URI, snapshot: EtheranaFileSnapshot): void {
		this._restoreEtheranaFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Etherana Coder: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._etheranaModelService.initializeModel(uri)
		await this._etheranaModelService.saveModel(uri) // save the URI
	}
	...
	public instantlyApplySearchReplaceBlocks({ uri, searchReplaceBlocks, explanation }: { uri: URI, searchReplaceBlocks: string, explanation?: string }) {
		// start diffzone
		...
		const onDone = () => {
			...
			onFinishEdit()
			if (explanation) {
				Object.values(diffZone._diffOfId).forEach(d => {
					if (!d.rationale) d.rationale = explanation;
				});
			}
			this._generateRationalesForDiffZone(diffZone)
	...
	public instantlyRewriteFile({ uri, newContent, explanation }: { uri: URI, newContent: string, explanation?: string }) {
		// start diffzone
		const res = this._startStreamingDiffZone({
			uri,
			streamRequestIdRef: { current: null },
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Etherana Coder: diff.type not recognized on: ${from}`)
		}
			...
			while (shouldSendAnotherMessage) {
					...
					for (let blockNum = currStreamingBlockNum; blockNum < blocks.length; blockNum += 1) {
						...
						if (!(blockNum in addedTrackingZoneOfBlockNum)) {
							...
							if (typeof originalBounds === 'string' || hasOverlap) {
								...
								// Error finding text - details removed from console
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Etherana Coder: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Etherana error: ${diff}.type not recognized`)
		}
	...
	private async _generateRationalesForDiffZone(diffZone: DiffZone) {
		const diffs = Object.values(diffZone._diffOfId);
		if (diffs.length === 0) return;

		const { model } = this._etheranaModelService.getModel(diffZone._URI)
		if (!model) return

		const language = model.getLanguageId()
		const modelSelection = this._settingsService.state.modelSelectionOfFeature['Chat'] // Use Chat model for rationales

		await Promise.all(diffs.map(async diff => {
			if (diff.rationale) return;

			try {
				let fullText = '';
				const originalCode = diff.type === 'insertion' ? '' : (diff as any).originalCode || '';
				const newCode = diff.type === 'deletion' ? '(deleted)' : (diff as any).code || '';

				this._llmMessageService.sendLLMMessage({
					messagesType: 'chatMessages',
					messages: [{ role: 'user', content: diffRationale_userMessage({ originalCode, newCode, language }) }],
					separateSystemMessage: diffRationale_systemMessage,
					modelSelection,
					modelSelectionOptions: undefined,
					overridesOfModel: undefined,
					safetyMode: 'observe',
					onText: (p) => { fullText = p.fullText; },
					onFinalMessage: () => {
						diff.rationale = fullText.trim().replace(/^"|"$/g, '');
						this._refreshStylesAndDiffsInURI(diffZone._URI);
					},
					onError: () => { },
					onAbort: () => { },
					logging: { loggingName: 'diff-rationale' }
				});
			} catch (e) {
				console.error('Error generating rationale:', e);
			}
		}));
	}
}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(ETHERANA_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(ETHERANA_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._etheranaCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._etheranaCommandBarService.onDidChangeState(e => {

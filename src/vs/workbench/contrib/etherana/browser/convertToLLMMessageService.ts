...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .etheranarules file):\n${aiInstructions}`)
...
export interface IConvertToLLMMessageService {
	...
	prepareLLMChatMessages: (opts: { chatMessages: ChatMessage[], safetyMode: SafetyMode, modelSelection: ModelSelection | null }) => Promise<{ messages: LLMChatMessage[], separateSystemMessage: string | undefined }>
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	// Read .etheranarules files from workspace folders
	private _getEtheranaRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let etheranaRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.etheranarules')
				const { model } = this.etheranaModelService.getModel(uri)
				if (!model) continue
				etheranaRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return etheranaRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .etheranarules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.etheranaSettingsService.state.globalSettings.aiInstructions;
		const etheranaRulesFileContent = this._getEtheranaRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (etheranaRulesFileContent) ans.push(etheranaRulesFileContent)
		return ans.join('\n\n')
	}


	// system message
	private _generateChatMessagesSystemMessage = async (safetyMode: SafetyMode, specialToolFormat: 'openai-style' | 'anthropic-style' | 'gemini-style' | undefined) => {
		...
		const directoryStr = await this.directoryStrService.getAllDirectoriesStr({
			cutOffMessage: safetyMode === 'agent' || safetyMode === 'edit' || safetyMode === 'observe' ?
		...
		const terminalMemory = await Promise.all(persistentTerminalIDs.map(async id => {
			try {
				const output = await this.terminalToolService.readTerminal(id, 50); // last 50 lines
				return { id, output };
			} catch (e) {
				return { id, output: '' };
			}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.etheranaSettingsService.state
		...
		const modelSelectionOptions = this.etheranaSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, safetyMode, modelSelection }) => {
		if (modelSelection === null) return { messages: [], separateSystemMessage: undefined }
		...
		const { disableSystemMessage } = this.etheranaSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(safetyMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.etheranaSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
...
// Relevant Context:
${messages.relevantContext.split('\n').map(line => `//${line}`).join('\n')}`}

...
import { mountEtheranaCommandBar } from './react/out/etherana-editor-widgets-tsx/index.js'
...
import { ETHERANA_ACCEPT_DIFF_ACTION_ID, ETHERANA_REJECT_DIFF_ACTION_ID, ETHERANA_GOTO_NEXT_DIFF_ACTION_ID, ETHERANA_GOTO_PREV_DIFF_ACTION_ID, ETHERANA_GOTO_NEXT_URI_ACTION_ID, ETHERANA_GOTO_PREV_URI_ACTION_ID, ETHERANA_ACCEPT_FILE_ACTION_ID, ETHERANA_REJECT_FILE_ACTION_ID, ETHERANA_ACCEPT_ALL_DIFFS_ACTION_ID, ETHERANA_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IEtheranaCommandBarService {
...
export const IEtheranaCommandBarService = createDecorator<IEtheranaCommandBarService>('EtheranaCommandBarService');
...
export class EtheranaCommandBarService extends Disposable implements IEtheranaCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'etherana.EtheranaCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._etheranaModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IEtheranaCommandBarService, EtheranaCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type EtheranaCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountEtheranaCommandBar(root, accessor, { uri, editor } satisfies EtheranaCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies EtheranaCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidAcceptDiffAction', 'Etherana Coder: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.EtheranaExtension,
			}
		});
	}
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(IEtheranaCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidRejectDiffAction', 'Etherana Coder: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidGoToNextDiffAction', 'Etherana Coder: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidGoToPrevDiffAction', 'Etherana Coder: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('voidGoToNextUriAction', 'Etherana Coder: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('voidGoToPrevUriAction', 'Etherana Coder: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('voidAcceptFileAction', 'Etherana Coder: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('voidRejectFileAction', 'Etherana Coder: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('voidAcceptAllDiffsAction', 'Etherana Coder: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('voidRejectAllDiffsAction', 'Etherana Coder: Reject All Diffs in All Files'),
			keybinding: {

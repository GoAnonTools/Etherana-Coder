...
export class ToolsService implements IToolsService {
	...
	private _ensureSafetyMode(action: 'read' | 'write' | 'terminal'): void {
		const mode = this.etheranaSettingsService.state.globalSettings.safetyMode;
		if (mode === 'observe' && (action === 'write' || action === 'terminal')) {
			throw new Error(`Action blocked: AI is in ${mode} mode and cannot perform ${action} actions.`);
		}
		if (mode === 'edit' && action === 'terminal') {
			throw new Error(`Action blocked: AI is in ${mode} mode and cannot perform ${action} actions.`);
		}
	}
	...
	constructor(
		...
		@IEtheranaModelService etheranaModelService: IEtheranaModelService,
		@IEditCodeService editCodeService: IEditCodeService,
		@ITerminalToolService private readonly terminalToolService: ITerminalToolService,
		@IEtheranaCommandBarService private readonly commandBarService: IEtheranaCommandBarService,
		...
		this.callTool = {
			read_file: async ({ uri, startLine, endLine, pageNumber }) => {
				this._ensureSafetyMode('read')
				await etheranaModelService.initializeModel(uri)
				const { model } = await etheranaModelService.getModelSafe(uri)
			...
			rewrite_file: async ({ uri, newContent, explanation }) => {
				this._ensureSafetyMode('write')
				await etheranaModelService.initializeModel(uri)
				...
				editCodeService.instantlyRewriteFile({ uri, newContent, explanation })
			...
			edit_file: async ({ uri, searchReplaceBlocks, explanation }) => {
				this._ensureSafetyMode('write')
				await etheranaModelService.initializeModel(uri)
				if (this.commandBarService.getStreamState(uri) === 'streaming') {
					throw new Error(`Another LLM is currently making changes to this file. Please stop streaming for now and ask the user to resume later.`)
				...
				await editCodeService.callBeforeApplyOrEdit(uri)
				editCodeService.instantlyApplySearchReplaceBlocks({ uri, searchReplaceBlocks, explanation })
			...
			run_command: async ({ command, cwd, terminalId }) => {
				this._ensureSafetyMode('terminal')
		...
		this.stringOfResult = {
			...
			search_in_file: (params, result) => {
				const { model } = etheranaModelService.getModel(params.uri)
			...
			edit_file: (params, result) => {
				const lintErrsString = (
					this.etheranaSettingsService.state.globalSettings.includeToolLintErrors ?
			...
			run_command: (params, result) => {
				...
				if (resolveReason.type === 'timeout') {
					return `${result_}\nTerminal command ran, but was automatically killed by Etherana after ${MAX_TERMINAL_INACTIVE_TIME}s of inactivity and did not finish successfully. To try with more time, open a persistent terminal and run the command there.`
				}

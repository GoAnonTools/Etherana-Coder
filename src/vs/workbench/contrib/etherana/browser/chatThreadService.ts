...
import { IFlightRecorderService } from './flightRecorderService.js';
import { ThreadStore } from './chat/ThreadStore.js';
import { SnapshotManager } from './chat/SnapshotManager.js';
import { ToolOrchestrator } from './chat/ToolOrchestrator.js';
import { LLMStreamer } from './chat/LLMStreamer.js';
import { ContextManager } from './chat/ContextManager.js';
import { MemoryManager } from './chat/MemoryManager.js';
import { ChatThreads, ThreadType, ThreadsState, IsRunningType, ThreadStreamState, WhenMounted, ChatMessage, StagingSelectionItem, CodespanLocationLink, ToolMessage, PlanEntry, CheckpointEntry } from '../common/chatThreadServiceTypes.js';
export { ChatThreads, ThreadType, ThreadsState, IsRunningType, ThreadStreamState, WhenMounted, ChatMessage, StagingSelectionItem, CodespanLocationLink, ToolMessage, PlanEntry, CheckpointEntry };
...
import { ITerminalMemoryService } from './terminalMemoryService.js';
...
import { ToolName, ToolCallParams, ToolResult, BuiltinToolCallParams } from '../common/toolsServiceTypes.js';
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/etheranaSettingsTypes.js';

import { getErrorMessage, RawToolParamsObj, RawToolCallObj, AnthropicReasoning } from '../common/sendLLMMessageTypes.js';
import { isABuiltinToolName, chat_userMessageContent } from '../common/prompt/prompts.js';
...
const newThreadObject = () => (new ThreadStore(null as any)).createNewThread()
...
export interface IChatThreadService {
	...
	// plan approval
	approvePlan(threadId: string, messageIdx: number): void;
	rejectPlan(threadId: string, messageIdx: number): void;

	// feature brief
	generateFeatureBrief(threadId: string): Promise<string>;

	focusCurrentChat: () => Promise<void>
	blurCurrentChat: () => Promise<void>
}

export const IChatThreadService = createDecorator<IChatThreadService>('etheranaChatThreadService');
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	private readonly _threadStore: ThreadStore;
	private readonly _snapshotManager: SnapshotManager;
	private readonly _toolOrchestrator: ToolOrchestrator;
	private readonly _llmStreamer: LLMStreamer;
	private readonly _memoryManager: MemoryManager;

	constructor(
		@IStorageService private readonly _storageService: IStorageService,
		@IEtheranaModelService private readonly _etheranaModelService: IEtheranaModelService,
		...
		@IFlightRecorderService private readonly _flightRecorderService: IFlightRecorderService,
		@ITerminalMemoryService private readonly _terminalMemoryService: ITerminalMemoryService,
		@IProjectMemoryService private readonly _projectMemoryService: IProjectMemoryService,
		@IEtheranaCommandBarService private readonly _etheranaCommandBarService: IEtheranaCommandBarService,
	) {
		super()
		this._threadStore = this._register(new ThreadStore(this._storageService));
		this._snapshotManager = new SnapshotManager(this._editCodeService, this._etheranaModelService);
		this._toolOrchestrator = new ToolOrchestrator(this._toolsService, this._mcpService);
		this._llmStreamer = new LLMStreamer(this._llmMessageService);
		this._memoryManager = new MemoryManager(this._settingsService, this._projectMemoryService, this._etheranaCommandBarService);

		this.state = { allThreads: {}, currentThreadId: null as unknown as string } // default state

		const readThreads = this._threadStore.readAllThreads() || {}
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart Etherana)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart Etherana), cancel that last tool
	...
	private _currentModelSelectionProps = (featureName: FeatureName = 'Chat') => {
		// these settings should not change throughout the loop (eg anthropic breaks if you change its thinking mode and it's using tools)
		const modelSelection = this._settingsService.state.modelSelectionOfFeature[featureName]
		const modelSelectionOptions = modelSelection ? this._settingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName] : undefined
		return { modelSelection, modelSelectionOptions, mode: featureName }
	}
	...
	rejectLatestToolRequest(threadId: string) {
		...
		// If there's an active auto-approval plan, clear it on rejection
		this._setThreadState(threadId, { autoApprovePlanId: null });
		
		this._setStreamState(threadId, undefined)
	}

	approvePlan(threadId: string, messageIdx: number) {
		const thread = this.state.allThreads[threadId];
		if (!thread) return;
		const plan = thread.messages[messageIdx];
		if (!plan || plan.role !== 'plan') return;

		this._editMessageInThread(threadId, messageIdx, { ...plan, isApproved: true });
		this._setThreadState(threadId, { autoApprovePlanId: plan.id });

		// If we are currently stalled at a tool_request for this plan, proceed
		if (this.streamState[threadId]?.isRunning === 'awaiting_user') {
			this.approveLatestToolRequest(threadId);
		}
	}

	rejectPlan(threadId: string, messageIdx: number) {
		const thread = this.state.allThreads[threadId];
		if (!thread) return;
		const plan = thread.messages[messageIdx];
		if (!plan || plan.role !== 'plan') return;
		
		this.abortRunning(threadId);
	}
	...
	async generateFeatureBrief(threadId: string): Promise<string> {
		const events = await this._flightRecorderService.getEvents(threadId);
		if (events.length === 0) return 'No major changes recorded in this session yet.';

		const summaryData = events
			.filter(e => e.status === 'success')
			.map(e => `- ${e.actionSummary} (${e.actionType})`)
			.join('\n');

		const prompt = `You are Ethereana Coder assistant. Create a concise, technical, yet punchy "What's New" brief for this coding session.
Focus on WHAT was achieved, not just the tools used.
Format it with a catchy title, a short summary paragraph, and then 3-5 bullet points of key changes.
...
${summaryData}`;
		...
		return new Promise((resolve) => {
			this._llmMessageService.sendLLMMessage({
				...
				safetyMode: null,
				modelSelection: this._currentModelSelectionProps().modelSelection,
				modelSelectionOptions: this._currentModelSelectionProps().modelSelectionOptions,
				overridesOfModel: undefined,
				logging: { loggingName: 'Generate Feature Brief' },
				onText: () => { },
				onFinalMessage: (res) => resolve(res.fullText || 'Could not generate brief.'),
				onError: (err) => resolve(`Error generating brief: ${err.message}`),
				onAbort: () => resolve('Generation aborted.')
			});
		});
	}
	...
	private _runToolCall = async (
		...
		if (!opts.preapproved) { // skip this if pre-approved
			...
			try {
				toolParams = this._toolOrchestrator.validateParams(toolName, opts.unvalidatedToolParams);
			}
			catch (error) {
			...
			const approvalType = this._toolOrchestrator.getApprovalType(toolName);
			if (approvalType) {
				const autoApprove = this._settingsService.state.globalSettings.autoApprove[approvalType]
				
				// Check for Plan approval
				const thread = this.state.allThreads[threadId];
				if (!thread) return { awaitingUserApproval: true };

				const lastPlan = findLast(thread.messages, m => m.role === 'plan') as PlanEntry | undefined;
				const isPlanApproved = lastPlan && lastPlan.isApproved && thread.state.autoApprovePlanId === lastPlan.id;
				const planIsHighRisk = lastPlan && lastPlan.risk === 'high';
				...
				// High risk always requires plan approval. 
				// Otherwise, respect auto-approve or already approved plan.
				if (planIsHighRisk && !isPlanApproved) {
					return { awaitingUserApproval: true };
				}

				if (!autoApprove && !isPlanApproved) {
					return { awaitingUserApproval: true }
				}
			}
		}
			...
			const safetyMode = this._settingsService.state.globalSettings.safetyMode;
			this._flightRecorderService.recordEvent({
				actionType: 'tool_call',
				threadId,
				messageId: toolId,
				actionSummary: `Calling tool: ${toolName}`,
				params: toolParams,
				status: 'running',
				safetyMode
			});
		...
		try {
			...
			if (interrupted) { return { interrupted: true } } // the tool result is added where we interrupt, not here
				...
				this._flightRecorderService.recordEvent({
					...
					threadId,
					messageId: toolId,
					actionSummary: `Tool error: ${toolName}`,
					params: toolParams,
					status: 'error',
					errorMessage: errorMessage,
					safetyMode: this._settingsService.state.globalSettings.safetyMode
				});
		...
		try {
			toolResultStr = this._toolOrchestrator.stringifyResult(toolName, toolResult);
		} catch (error) {
		...
		this._flightRecorderService.recordEvent({
			...
			threadId,
			messageId: toolId,
			actionSummary: `Tool success: ${toolName}`,
			params: toolParams,
			result: toolResult,
			status: 'success',
			safetyMode: this._settingsService.state.globalSettings.safetyMode
	...
	private async _runChatAgent({
		...
		const { safetyMode } = this._settingsService.state.globalSettings // should not change as we loop even if user changes it, so it goes here
		...
		// Phase 3: Fetch terminal memory if enabled
		let terminalContext: { content: string, metadata: any } | null = null;
		if (modelSelection) {
			terminalContext = await this._terminalMemoryService.getRecentTerminalContext({ providerName: modelSelection.providerName });
		}

		// Phase 4: Fetch project memory if enabled
		let projectMemoryContext: { content: string, metadata: any } | null = null;
		if (modelSelection) {
			projectMemoryContext = this._memoryManager.getProjectMemoryContext({ mode });
		}
		...
		while (shouldSendAnotherMessage) {
			...
			while (shouldRetryLLM) {
				...
				const modelMetadata = modelSelection ? this._settingsService.getModelMetadata(modelSelection.providerName, modelSelection.modelName) : null;

				this._flightRecorderService.recordEvent({
					actionType: 'chat',
					threadId,
					messageId: currentChatId,
					actionSummary: `AI request (${safetyMode})`,
					status: 'running',
					safetyMode,
					params: {
						modelSelection,
						modelMetadata,
						llmMessageLogging: true,
						terminalContextMetadata: terminalContext?.metadata,
						projectMemoryMetadata: projectMemoryContext?.metadata
					}
				});

				// Inject terminal context into messages if available (only on first message of loop)
				const finalMessages = [...messages];
				// Injection of terminal context
				if (terminalContext?.content?.trim() && nMessagesSent === 1) {
					ContextManager.injectTerminalContext(finalMessages, terminalContext.content);
				}

				// Injection of project memory context
				if (projectMemoryContext?.content?.trim() && nMessagesSent === 1) {
					ContextManager.injectProjectMemoryContext(finalMessages, projectMemoryContext.content);
				}

				const llmCancelToken = this._llmStreamer.sendLLMMessage({
					messages: finalMessages,
					modelSelection,
					modelSelectionOptions,
					safetyMode: safetyMode as any,
					...
					onFinalMessage: async ({ fullText, fullReasoning, toolCall, anthropicReasoning, }) => {
						this._flightRecorderService.recordEvent({
							...
							threadId,
							messageId: currentChatId,
							actionSummary: 'AI response received',
							result: { fullText, toolCall: toolCall?.name },
							status: 'success',
							safetyMode
						});
					...
					onAbort: () => {
						...
						this._metricsService.capture('Agent Loop Done (Aborted)', { nMessagesSent, safetyMode })
					},
				})
		...
		this._metricsService.capture('Agent Loop Done', { nMessagesSent, safetyMode })
	}
	...
	private _editMessageInThread(threadId: string, messageIdx: number, newMessage: ChatMessage,) {
		...
		this._threadStore.storeAllThreads(newThreads)
	...
	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const thread = this.state.allThreads[threadId]
		if (!thread) return
		const { etheranaFileSnapshotOfURI } = this._snapshotManager.computeNewCheckpointInfo(thread) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			etheranaFileSnapshotOfURI: etheranaFileSnapshotOfURI ?? {},
			userModifications: { etheranaFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		const thread = this.state.allThreads[threadId]
		if (!thread) return
		const { model } = this._etheranaModelService.getModel(uri)
		if (!model) return // should never happen
		const diffAreasSnapshot = this._editCodeService.getEtheranaFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			etheranaFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { etheranaFileSnapshotOfURI: {} },
		})
	}
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { etheranaFileSnapshotOfURI: etheranaFileSnapshotOfURI ?? {}, },
		})
	}
	...
	jumpToCheckpointBeforeMessageIdx({ threadId, messageIdx, jumpToUserModified }: { threadId: string, messageIdx: number, jumpToUserModified: boolean }) {
		...
		const c = this._snapshotManager.getCheckpointBeforeMessage(thread, messageIdx)
		...
		if (toIdx < fromIdx) {
			const { lastIdxOfURI } = this._snapshotManager.getCheckpointsBetween(thread, toIdx + 1, fromIdx)
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					const res = this._snapshotManager.getCheckpointInfo(message, fsPath, { includeUserModifiedChanges: jumpToUserModified })
					if (!res) continue
					const { etheranaFileSnapshot } = res
					if (!etheranaFileSnapshot) continue
					this._snapshotManager.restoreSnapshot(fsPath, etheranaFileSnapshot)
					break
				}
			}
		}
		...
		if (toIdx > fromIdx) {
			const { lastIdxOfURI } = this._snapshotManager.getCheckpointsBetween(thread, fromIdx + 1, toIdx)
			for (const fsPath in lastIdxOfURI) {
	...
	private async _addUserMessageAndStreamResponse({ userMessage, _chatSelections, threadId }: { userMessage: string, _chatSelections?: StagingSelectionItem[], threadId: string }) {
		...
		this._flightRecorderService.recordEvent({
			actionType: 'chat',
			threadId,
			actionSummary: 'User message',
			params: { userMessage: instructions },
			status: 'success',
			safetyMode: this._settingsService.state.globalSettings.safetyMode
		});
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._etheranaModelService.getModelSafe(uri)
	...
	deleteThread(threadId: string): void {
		const allThreads = this._threadStore.deleteThread(this.state.allThreads, threadId);
		this._setState({ allThreads });
	}

	duplicateThread(threadId: string): void {
		const { newThreads, newThreadId } = this._threadStore.duplicateThread(this.state.allThreads, threadId);
		this._setState({ allThreads: newThreads });

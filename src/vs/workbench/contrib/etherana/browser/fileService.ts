...
class FilePromptActionService extends Action2 {
	private static readonly ETHERANA_COPY_FILE_PROMPT_ID = 'etherana.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.ETHERANA_COPY_FILE_PROMPT_ID,
			title: localize2('voidCopyPrompt', 'Etherana Coder: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await etheranaModelService.getModelSafe(uri)).model?.getLanguageId() || '',

...
import { ETHERANA_VIEW_CONTAINER_ID, ETHERANA_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { ETHERANA_TOGGLE_SETTINGS_ACTION_ID } from './etheranaSettingsPane.js';
...
const ETHERANA_OPEN_SIDEBAR_ACTION_ID = 'etherana.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: ETHERANA_OPEN_SIDEBAR_ACTION_ID, title: localize2('voidOpenSidebar', 'Etherana Coder: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(ETHERANA_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('voidCmdL', 'Etherana Coder: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(ETHERANA_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(ETHERANA_OPEN_SIDEBAR_ACTION_ID)
		}
...
const ETHERANA_CMD_SHIFT_L_ACTION_ID = 'etherana.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: ETHERANA_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', ETHERANA_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(ETHERANA_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'etherana.settingsAction',
			title: `Etherana Coder's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(ETHERANA_TOGGLE_SETTINGS_ACTION_ID)
	}
})

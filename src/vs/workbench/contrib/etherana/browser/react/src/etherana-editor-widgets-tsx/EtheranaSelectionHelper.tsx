...
import '../styles.css'
import { ETHERANA_CTRL_K_ACTION_ID, ETHERANA_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { EtheranaSelectionHelperProps } from '../../../../../../contrib/etherana/browser/etheranaSelectionHelperWidget.js';
import { ETHERANA_OPEN_SETTINGS_ACTION_ID } from '../../../etheranaSettingsPane.js';


export const EtheranaSelectionHelperMain = (props: EtheranaSelectionHelperProps) => {
	...
	>
		<EtheranaSelectionHelper {...props} />
	</div>
}



const EtheranaSelectionHelper = ({ rerenderKey }: EtheranaSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(ETHERANA_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(ETHERANA_CTRL_K_ACTION_ID)

	const dividerHTML = <div className='w-[0.5px] bg-etherana-border-3'></div>
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === ETHERANA_CTRL_L_ACTION_ID || e.commandId === ETHERANA_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				onClick={() => {
					commandService.executeCommand(ETHERANA_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				className='flex items-center px-2 py-1.5 cursor-pointer'
				onClick={() => {
					commandService.executeCommand(ETHERANA_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		<div
			className='flex items-center px-0.5 cursor-pointer'
			onClick={() => {
	...
	const moreOptionsHTML = <>
		<div
			className='flex items-center px-2 py-1.5 cursor-pointer'
			onClick={() => {
				commandService.executeCommand(ETHERANA_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
	...
	return <div className='pointer-events-auto select-none z-[1000] rounded-sm shadow-md flex flex-nowrap text-nowrap border border-etherana-border-3 bg-etherana-bg-2 transition-all duration-200'>

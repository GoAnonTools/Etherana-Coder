...
import { EtheranaCommandBarProps } from '../../../etheranaCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	ETHERANA_GOTO_NEXT_DIFF_ACTION_ID,
	ETHERANA_GOTO_PREV_DIFF_ACTION_ID,
	ETHERANA_GOTO_NEXT_URI_ACTION_ID,
	ETHERANA_GOTO_PREV_URI_ACTION_ID,
	ETHERANA_ACCEPT_FILE_ACTION_ID,
	ETHERANA_REJECT_FILE_ACTION_ID,
	ETHERANA_ACCEPT_ALL_DIFFS_ACTION_ID,
	ETHERANA_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const EtheranaCommandBarMain = ({ uri, editor }: EtheranaCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@etherana-scope ${isDark ? 'dark' : ''}`}
	>
		<EtheranaCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const EtheranaCommandBar = ({ uri, editor }: EtheranaCommandBarProps) => {
	...
	const etheranaModelService = accessor.get('IEtheranaModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(ETHERANA_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(ETHERANA_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(ETHERANA_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(ETHERANA_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(ETHERANA_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(ETHERANA_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(ETHERANA_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(ETHERANA_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-etherana-bg-2 shadow-md border border-etherana-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-etherana-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-etherana-bg-2 rounded shadow-md border border-etherana-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-etherana-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-etherana-bg-2 rounded shadow-md border border-etherana-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-etherana-border-2 [&>*:last-child]:border-r-0">

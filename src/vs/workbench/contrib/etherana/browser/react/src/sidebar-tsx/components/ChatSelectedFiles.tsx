...
import { getBasename, getRelative, voidOpenFileFn } from './ChatUtils.js';
import { IconX } from './ChatIcons.js';
...
export const SelectedFiles = (
	...
	const modelReferenceService = accessor.get('IEtheranaModelService');
			...
			{allSelections.map((selection, i) => {
				...
				const thisKey = selection.type === 'CodeSelection' ? selection.type + selection.language + selection.range + selection.state.wasAddedAsCurrentFile + selection.uri.fsPath
					: selection.type === 'File' ? selection.type + selection.language + selection.state.wasAddedAsCurrentFile + selection.uri.fsPath
						: selection.type === 'Folder' ? selection.type + selection.state + selection.uri.fsPath
							: i;
				...
				return (
					<div key={thisKey} className={`flex flex-col space-y-[1px]`}>
						...
						>
							<div
								className={`flex items-center gap-1 relative px-1 w-fit h-fit select-none text-xs text-nowrap border rounded-sm transition-all duration-150 ${isThisSelectionProspective ? 'bg-etherana-bg-1 text-etherana-fg-3 opacity-80 border-etherana-border-2' : 'bg-etherana-bg-1 hover:brightness-95 text-etherana-fg-1 border-etherana-border-1'} hover:border-etherana-border-1`}
								onClick={() => {
							...
							>
								...
								{selection.type === 'File' && selection.state.wasAddedAsCurrentFile && messageIdx === undefined && currentURI?.fsPath === selection.uri.fsPath && (
									<span className={`text-[8px] opacity-60 text-etherana-fg-4`}>(Current File)</span>
								)}
								{type === 'staging' && !isThisSelectionProspective && (
									<div className='cursor-pointer z-1 self-stretch flex items-center justify-center' onClick={(e) => { e.stopPropagation(); setSelections([...selections.slice(0, i), ...selections.slice(i + 1)]); }}>

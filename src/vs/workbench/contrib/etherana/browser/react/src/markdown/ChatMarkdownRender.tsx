...
import { getBasename, getRelative, voidOpenFileFn } from '../sidebar-tsx/components/ChatUtils.js'
...
const Codespan = ({ text, className, onClick, tooltip }: { text: string, className?: string, onClick?: () => void, tooltip?: string }) => {
	...
	return <code
		className={`font-mono font-medium rounded-sm bg-etherana-bg-1 px-1 ${className}`}
		onClick={onClick}
		{...tooltip ? {
...
const RenderToken = ({ token, inPTag, codeURI, chatMessageLocation, tokenIdx, ...options }: { token: Token | string, inPTag?: boolean, codeURI?: URI, chatMessageLocation?: ChatMessageLocation, tokenIdx: string, } & RenderTokenOptions): React.ReactNode => {
	...
	if (t.type === 'table') {
		...
		// return (
		// 	<div>
		// 		<table className={'min-w-full border border-etherana-bg-2'}>
		// 			<thead>
		// 				<tr className='bg-etherana-bg-1'>
		// 					{t.header.map((cell: any, index: number) => (
		// 						<th
		// 							key={index}
		// 							className='px-4 py-2 border border-etherana-bg-2 font-semibold'
		...
		// 					<tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-etherana-bg-1'}>
		// 						{row.map((cell: any, cellIndex: number) => (
		// 							<td
		// 								key={cellIndex}
		// 								className={'px-4 py-2 border border-etherana-bg-2'}
	...
	if (t.type === 'link') {
		return (
			<a
				...
				className='underline cursor-pointer hover:brightness-90 transition-all duration-200 text-etherana-fg-2'
			>
				{t.text}
			</a>
		)

...
import { X, RefreshCw, Loader2, Check, Asterisk, Plus, Trash2 } from 'lucide-react'
import { URI } from '../../../../../../../base/common/uri.js'
import { ModelDropdown } from './ModelDropdown.js'
import { ChatMarkdownRender } from '../markdown/ChatMarkdownRender.js'
import { WarningBox } from './WarningBox.js'
import { ProjectMemoryPanel } from './ProjectMemoryPanel.js'
import { PrivacyDashboard } from './PrivacyDashboard.js'
import { os } from '../../../../common/helpers/systemInfo.js'
import { IconLoading } from '../sidebar-tsx/components/ChatIcons.js'
...
const ButtonLeftTextRightOption = ({ text, leftButton }: { text: string, leftButton?: React.ReactNode }) => {
	...
	return <div className='flex items-center text-etherana-fg-3 px-3 py-0.5 rounded-sm overflow-hidden gap-2'>
...
const ConfirmButton = ({ children, onConfirm, className }: { children: React.ReactNode, onConfirm: () => void, className?: string }) => {
	...
	return (
		<div ref={ref} className={`inline-block`}>
			<EtheranaButtonBgDarken className={className} onClick={() => {
				if (!confirm) {
					setConfirm(true);
				} else {
					onConfirm();
...
const SimpleModelSettingsDialog = ({
	...
	const sourcecodeOverridesLink = `https://github.com/etherana/etherana/blob/2e5ecb291d33afbe4565921664fb7e183189c1c5/src/vs/workbench/contrib/etherana/common/modelCapabilities.ts#L146-L172`
	...
	return (
		...
		>
			{/* MODAL */}
			<div
				className="bg-etherana-bg-1 rounded-md p-4 max-w-xl w-full shadow-xl overflow-y-auto max-h-[90vh]"
			...
			>
				<div className="flex justify-between items-center mb-4">
					...
					<button
						onClick={onClose}
						className="text-etherana-fg-3 hover:text-etherana-fg-1"
					>
						<X className="size-5" />
					</button>
				</div>
				...
				<div className="flex items-center gap-2 mb-4">
					<EtheranaSwitch size='xs' value={overrideEnabled} onChange={setOverrideEnabled} />
					<span className="text-etherana-fg-3 text-sm">Override model defaults</span>
				</div>

				{/* Informational link */}
				{overrideEnabled && <div className="text-sm text-etherana-fg-3 mb-4">
				...
				<textarea
					key={overrideEnabled + ''}
					ref={textAreaRef}
					className={`w-full min-h-[200px] p-2 rounded-sm border border-etherana-border-2 bg-etherana-bg-2 resize-none font-mono text-sm ${!overrideEnabled ? 'text-etherana-fg-3' : ''}`}
				...
				<div className="flex justify-end gap-2 mt-4">
					<EtheranaButtonBgDarken onClick={onClose} className="px-3 py-1">
						Cancel
					</EtheranaButtonBgDarken>
					<EtheranaButtonBgDarken
						onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	const modelDump: (EtheranaStatefulModelInfo & { providerName: ProviderName, providerEnabled: boolean })[] = []
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='etherana-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='etherana-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
				...
				<div className="flex items-center gap-2 w-fit">
					...
					{disabled ? null : (
						<div className="w-5 flex items-center justify-center">
							...
							>
								<Plus size={12} className="text-etherana-fg-3 opacity-50" />
							</button>
						</div>
					)}
					...
					{/* Delete button (Trash). Show for all models. */}
					<div className="w-5 flex items-center justify-center">
						<button
							onClick={() => { settingsStateService.deleteModel(providerName, modelName); }}
							data-tooltip-id='etherana-tooltip'
							data-tooltip-place='right'
							data-tooltip-content='Delete Model'
							className="transition-opacity"
						>
							<Trash2 size={12} className="text-red-500 opacity-70 hover:opacity-100" />
						</button>
					</div>
		...
		{showCheckmark ? (
				...
				<form className="flex items-center gap-2">
					...
					<ErrorBoundary>
						<EtheranaCustomDropdownBox
							...
							className="max-w-32 mx-2 w-full resize-none bg-etherana-bg-1 text-etherana-fg-1 placeholder:text-etherana-fg-3 border border-etherana-border-2 focus:border-etherana-border-1 py-1 px-2 rounded"
			...
			<div
				className="text-etherana-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer mt-4"
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	if (typeof settingValue !== 'string') {
		// console.log removed: use proper error handling instead of console.log
		return
	}
	...
	const handleChangeValue = useCallback((newVal: string) => {
		etheranaSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [etheranaSettingsService, providerName, settingName]);
...
// 	const needsModel = isProviderNameDisabled(providerName, etheranaSettingsState) === 'addModel'

// 	// const accessor = useAccessor()
// 	// const etheranaSettingsService = accessor.get('IEtheranaSettingsService')

// 	// const { enabled } = etheranaSettingsState.settingsOfProvider[providerName]
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = etheranaSettingsService.state.settingsOfProvider[providerName].enabled
// 						etheranaSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [etheranaSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const EtheranaProviderSettings = ({ providerNames }: { providerNames: ProviderName[] }) => {
	return <>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	const enabled = etheranaSettingsState.globalSettings[settingName]

	return <ButtonLeftTextRightOption
		leftButton={<EtheranaSwitch
			size='xxs'
			value={enabled}
			onChange={(newVal) => {
				etheranaSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	return <EtheranaInputBox2
		...
		initValue={etheranaSettingsState.globalSettings.aiInstructions}
		...
		onChangeText={(newText) => {
			etheranaSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const TerminalMemorySettings = () => {
	...
	const settingsState = useSettingsState();
	const settings = settingsState.globalSettings.terminalMemory || defaultGlobalSettings.terminalMemory;
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-2">
					<EtheranaSwitch
						...
						onChange={(val) => settingsService.setGlobalSetting('terminalMemory', { ...settings, enabled: val })}
					/>
					<span className="text-sm font-medium">Enable Terminal Memory</span>
				</div>
			</div>

			<p className="text-xs text-etherana-fg-3">
				AI can automatically understand recent terminal output (errors, results, logs) for context. Potential secrets are redacted before sending, but redaction may not catch everything.
			</p>

			{settings.enabled && (
				<div className="pl-6 space-y-4 border-l border-etherana-border-3">
					<div className="space-y-1">
						<label className="text-xs text-etherana-fg-2">Max lines to include</label>
						<EtheranaSlider
							min={10} max={200} step={10}
							value={settings.maxLines}
							onChange={(val: number) => settingsService.setGlobalSetting('terminalMemory', { ...settings, maxLines: val })}
						/>
						<div className="text-xs text-etherana-fg-1">{settings.maxLines} lines</div>
					</div>
					...
					<ButtonLeftTextRightOption
						leftButton={
							<EtheranaSwitch
								size='xxs'
								value={settings?.redactSecrets ?? true}
								onChange={(val) => settingsService.setGlobalSetting('terminalMemory', { ...settings, redactSecrets: val })}
							/>
						}
						text="Redact potential secrets and tokens (recommended)"
					/>
					...
					<ButtonLeftTextRightOption
						leftButton={
							<EtheranaSwitch
								size='xxs'
								value={settings?.askBeforeCloud ?? true}
								onChange={(val) => settingsService.setGlobalSetting('terminalMemory', { ...settings, askBeforeCloud: val })}
							/>
						}
						text="Ask before sending terminal context to cloud models"
					/>
				</div>
			)}
		</div>
...
const ProjectMemorySettings = () => {
	const etheranaSettingsService = useAccessor().get('IEtheranaSettingsService');
	const settingsState = useSettingsState();
	const settings = settingsState.globalSettings.projectMemory;
		...
		<div className="space-y-4">
			<div className="flex items-center gap-2">
				<EtheranaSwitch
					size="xs"
					value={settings.enabled}
					onChange={(val) => etheranaSettingsService.setGlobalSetting('projectMemory', { ...settings, enabled: val })}
				/>
				<span className="text-sm font-medium">Enable Project Memory</span>
			</div>
			{settings.enabled && (
				<div className="pl-6 space-y-3">
					<div className="flex items-center justify-between">
						<span className="text-sm text-etherana-fg-2">Include in Chat</span>
						<EtheranaSwitch
							size="xs"
							value={settings.includeInChat}
							onChange={(val) => etheranaSettingsService.setGlobalSetting('projectMemory', { ...settings, includeInChat: val })}
						/>
					</div>
					<div className="flex items-center justify-between">
						<span className="text-sm text-etherana-fg-2">Include in Gather</span>
						<EtheranaSwitch
							size="xs"
							value={settings.includeInGather}
							onChange={(val) => etheranaSettingsService.setGlobalSetting('projectMemory', { ...settings, includeInGather: val })}
						/>
					</div>
					<div className="flex items-center justify-between">
						<span className="text-sm text-etherana-fg-2">Include in Agent</span>
						<EtheranaSwitch
							size="xs"
							value={settings.includeInAgent}
							onChange={(val) => etheranaSettingsService.setGlobalSetting('projectMemory', { ...settings, includeInAgent: val })}
						/>
					</div>
					<div className="flex items-center justify-between">
						<span className="text-sm text-etherana-fg-2">Include in Autocomplete</span>
						<EtheranaSwitch
							size="xs"
							value={settings.includeInAutocomplete}
							onChange={(val) => etheranaSettingsService.setGlobalSetting('projectMemory', { ...settings, includeInAutocomplete: val })}
						/>
					</div>
					<div className="flex items-center justify-between">
						<span className="text-sm text-etherana-fg-2">Prefer User-Confirmed</span>
						<EtheranaSwitch
							size="xs"
							value={settings.preferUserConfirmed}
							onChange={(val) => etheranaSettingsService.setGlobalSetting('projectMemory', { ...settings, preferUserConfirmed: val })}
						/>
					</div>
					<div className="flex items-center justify-between">
						<span className="text-sm text-etherana-fg-2">Max Entries per Request</span>
						<input
							type="number"
							min={1}
							max={20}
							value={settings.maxEntriesPerRequest}
							onChange={(e) => etheranaSettingsService.setGlobalSetting('projectMemory', { ...settings, maxEntriesPerRequest: parseInt(e.target.value) })}
							className="w-16 bg-etherana-bg-1 border border-etherana-border-1 rounded px-2 py-0.5 text-sm outline-none focus:border-[#0e70c0]"
						/>
					</div>
				</div>
			)}
...
const ModelRouter = () => {
	...
	const features: FeatureName[] = ['Chat', 'Gather', 'Agent', 'Autocomplete', 'Review'];

	return (
		<div className="space-y-2">
			{features.map(f => {
				const selection = settingsState.modelSelectionOfFeature[f];
				const metadata = selection ? settingsService.getModelMetadata(selection.providerName, selection.modelName) : null;

				return (
					<div key={f} className="flex items-center justify-between p-2 rounded-sm hover:bg-black/5 dark:hover:bg-white/5">
						<div className="flex flex-col">
							<span className="text-sm font-medium">{displayInfoOfFeatureName(f)}</span>
							{metadata && (
								<div className="flex gap-1 mt-0.5">
									<span className={`text-[10px] px-1 rounded-full ${metadata.privacyType === 'local' ? 'bg-green-500/20 text-green-600' : 'bg-blue-500/20 text-blue-600'}`}>
										{metadata.privacyLabel.toUpperCase()}
									</span>
									<span className={`text-[10px] px-1 rounded-full ${metadata.costTier === 'paid' ? 'bg-amber-500/20 text-amber-600' : (metadata.costTier === 'free' || metadata.costTier === 'free-tier') ? 'bg-green-500/20 text-green-600' : 'bg-gray-500/20 text-gray-600'}`}>
										{metadata.costLabel.toUpperCase()}
									</span>
								</div>
							)}
						</div>
						<ModelDropdown featureName={f} className="text-xs bg-transparent" />
					</div>
				);
			})}
		</div>
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		etheranaSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [etheranaSettingsService])

	return <EtheranaCustomDropdownBox
		className='text-xs text-etherana-fg-3 bg-etherana-bg-1 border border-etherana-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={etheranaSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-etherana-fg-3 text-sm list-decimal select-text'>
		<div className=''><ChatMarkdownRender string={`Ollama Setup Instructions`} chatMessageLocation={undefined} /></div>
		<div className='pl-6'><ChatMarkdownRender string={`1. Download [Ollama](https://ollama.com/download).`} chatMessageLocation={undefined} /></div>
		<div className='pl-6'><ChatMarkdownRender string={`2. Open your terminal.`} chatMessageLocation={undefined} /></div>
		<div
			className='pl-6 flex items-center w-fit'
			data-tooltip-id='etherana-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className='pl-6'><ChatMarkdownRender string={`Etherana automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	const etheranaSettingsService = accessor.get('IEtheranaSettingsService')
	return <div
		className={`text-etherana-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer ${className}`}
		onClick={() => { etheranaSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		etheranaSettingsService.setGlobalSetting('autoApprove', {
			...etheranaSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [etheranaSettingsService, metricsService])

	return <>
		<EtheranaSwitch
			size={size}
			value={etheranaSettingsState.globalSettings.autoApprove[approvalType] ?? false}
			onChange={(newVal) => onToggleAutoApprove(approvalType, newVal)}
		/>
		<span className="text-etherana-fg-3 text-xs">{desc}</span>
	</>
}
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: { fromEditor?: TransferEditorType, className?: string }) => {
	...
	return <>
		<EtheranaButtonBgDarken className={`max-w-48 p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	const etheranaSettings = useSettingsState()
	const isOn = etheranaSettings.mcpUserStateOfName[name]?.isOn

	const removeUniquePrefix = (name: string) => name.split('_').slice(1).join('_')

	return (
		<div className="border border-etherana-border-2 bg-etherana-bg-1 py-3 px-4 rounded-sm my-2">
			<div className="flex items-center justify-between">
						...
						${server.status === 'success' ? 'bg-green-500'
							: server.status === 'error' ? 'bg-red-500'
								: server.status === 'loading' ? 'bg-yellow-500'
									: server.status === 'offline' ? 'bg-etherana-fg-3'
										: ''}
					`}></div>

					{/* Server name */}
					<div className="text-sm font-medium text-etherana-fg-1">{name}</div>
				</div>
					...
					<div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
						{(server.tools ?? []).length > 0 ? (
							(server.tools ?? []).map((tool: { name: string; description?: string }) => (
								<span
									key={tool.name}
									className="px-2 py-0.5 bg-etherana-bg-2 text-etherana-fg-3 rounded-sm text-xs"
							...
							<span className="text-xs text-etherana-fg-3">No tools available</span>
						)}
					</div>
				</div>
			)}
			...
			{isOn && server.command && (
				<div className="mt-3">
					<div className="text-xs text-etherana-fg-3 mb-1">Command:</div>
					<div className="px-2 py-1 bg-etherana-bg-2 text-xs font-mono overflow-x-auto whitespace-nowrap text-etherana-fg-2 rounded-sm">
						{server.command}
					</div>
				</div>
			)}
...
const MCPServersList = () => {
	...
	if (mcpServiceState.error) {
		content = <div className="text-etherana-fg-3 text-sm mt-2">
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'etherana-chats.json'
		}
		else if (t === 'Settings') {
			// Export user settings
			dataStr = JSON.stringify(etheranaSettingsService.state, null, 2)
			downloadName = 'etherana-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					etheranaSettingsService.dangerousSetState(json as any)
				}
	...
	return (
		<div className={`@@etherana-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%', overflow: 'auto' }}>
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`Etherana Coder's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-etherana-fg-3 mb-2`}>{`Etherana can access any model that you host locally. We automatically detect your local models by default.`}</h3>

									<div className='opacity-80 mb-4'>
										<OllamaSetupInstructions sayWeAutoDetect={true} />
									</div>

									<EtheranaProviderSettings providerNames={localProviderNames} />
								</ErrorBoundary>
							</div>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-etherana-fg-3 mb-2`}>{`Etherana can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>

									<EtheranaProviderSettings providerNames={nonlocalProviderNames} />
								</ErrorBoundary>
							</div>
							...
							<div className={shouldShowTab('featureOptions') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<div className="flex flex-col gap-y-8 my-4">
										...
										<ErrorBoundary>
											<div>
												<h4 className={`text-base`}>Terminal Memory</h4>
												<div className="text-sm text-etherana-fg-3 mt-1">
													Control how much terminal context is provided to the AI.
												</div>
												<div className="my-4">
													<TerminalMemorySettings />
												</div>
											</div>
										</ErrorBoundary>
										...
										<ErrorBoundary>
											<div>
												<h4 className={`text-base`}>Project Memory Options</h4>
												<div className="text-sm text-etherana-fg-3 mt-1">
													Fine-tune how project-specific knowledge is used by the AI.
												</div>
												<div className="my-4">
										...
										{/* Per-Feature Model Router */}
										<ErrorBoundary>
											<div>
												<h4 className={`text-base`}>Model Router</h4>
												<div className="text-sm text-etherana-fg-3 mt-1">
													Route different tasks to different models to optimize for cost, speed, or intelligence.
												</div>
												<div className="my-4">
													<ModelRouter />
												</div>
											...
											<div>
												...
												<div className='text-sm text-etherana-fg-3 mt-1'>
													...
													<span
														...
														data-tooltip-class-name='max-w-[20px]'
													>
												...
												<div className='my-2'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<EtheranaSwitch
																...
																onChange={(newVal) => etheranaSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
															<span className='text-etherana-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
														</div>
													</ErrorBoundary>
													...
													<ErrorBoundary>
														<div className={`my-2 ${!settingsState.globalSettings.enableAutocomplete ? 'hidden' : ''}`}>
															<ModelDropdown featureName={'Autocomplete'} className='text-xs text-etherana-fg-3 bg-etherana-bg-1 border border-etherana-border-1 rounded p-0.5 px-1' />
														</div>
													</ErrorBoundary>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-etherana-fg-3 mt-1'>Settings that control the behavior of the Apply button.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<EtheranaSwitch
															...
															onChange={(newVal) => etheranaSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
														<span className='text-etherana-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncApplyToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'Apply'} className='text-xs text-etherana-fg-3 bg-etherana-bg-1 border border-etherana-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
										...
										<div>
											...
											<div className='text-sm text-etherana-fg-3 mt-1'>{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<EtheranaSwitch
															...
															onChange={(newVal) => etheranaSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
														<span className='text-etherana-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
													</div>
												</ErrorBoundary>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<EtheranaSwitch
															...
															onChange={(newVal) => etheranaSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
														<span className='text-etherana-fg-3 text-xs pointer-events-none'>Auto-accept LLM changes</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-etherana-fg-3 mt-1'>{`Settings that control the visibility of Etherana suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<EtheranaSwitch
															...
															onChange={(newVal) => etheranaSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
														<span className='text-etherana-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-etherana-fg-3 mt-1'>Settings that control the behavior of the commit message generator.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<EtheranaSwitch
															...
															onChange={(newVal) => etheranaSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
														<span className='text-etherana-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncSCMToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncSCMToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'SCM'} className='text-xs text-etherana-fg-3 bg-etherana-bg-1 border border-etherana-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-etherana-fg-3 mb-4'>{`Transfer your editor settings into Etherana.`}</h4>
								...
								<div>
									...
									<h4 className='text-etherana-fg-3 mb-4'>{`Transfer Etherana Coder's settings and chats in and out of Etherana.`}</h4>
									<div className='flex flex-col gap-8'>
										{/* Settings Subcategory */}
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											<input key={2 * s} ref={fileInputSettingsRef} type='file' accept='.json' className='hidden' onChange={handleUpload('Settings')} />
											<EtheranaButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputSettingsRef.current?.click() }}>
												Import Settings
											</EtheranaButtonBgDarken>
											<EtheranaButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Settings')}>
												Export Settings
											</EtheranaButtonBgDarken>
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { etheranaSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<EtheranaButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputChatsRef.current?.click() }}>
												Import Chats
											</EtheranaButtonBgDarken>
											<EtheranaButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Chats')}>
												Export Chats
											</EtheranaButtonBgDarken>
								...
								<div>
									...
									<h4 className={`text-etherana-fg-3 mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>

									<ErrorBoundary>
										<div className='flex flex-col gap-2 justify-center max-w-48 w-full'>
											<EtheranaButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openSettings') }}>
												General Settings
											</EtheranaButtonBgDarken>
											<EtheranaButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openGlobalKeybindings') }}>
												Keyboard Settings
											</EtheranaButtonBgDarken>
											<EtheranaButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.selectTheme') }}>
												Theme Settings
											</EtheranaButtonBgDarken>
											<EtheranaButtonBgDarken className='px-4 py-1' onClick={() => { nativeHostService.showItemInFolder(environmentService.logsHome.fsPath) }}>
												Open Logs
											</EtheranaButtonBgDarken>
										</div>
									</ErrorBoundary>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-etherana-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep Etherana running smoothly. You may opt out below. Regardless of this setting, Etherana never sees your code, messages, or API keys.</h4>
									...
									<div className='my-2'>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2 my-2'>
												...
												/>
												<span className='text-etherana-fg-3 text-xs pointer-events-none'>{'Opt-out (requires restart)'}</span>
											</div>
										</ErrorBoundary>
									</div>
								</div>
...
Use Model Context Protocol to provide Agent mode with more tools.
							`} chatMessageLocation={undefined} />
									...
									<div className='my-2'>
										<EtheranaButtonBgDarken className='px-4 py-1 w-full max-w-48' onClick={async () => { await mcpService.revealMCPConfigFile() }}>
											Add MCP Server
										</EtheranaButtonBgDarken>
									</div>

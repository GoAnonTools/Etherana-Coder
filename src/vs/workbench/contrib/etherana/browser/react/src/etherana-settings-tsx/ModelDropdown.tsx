...
import { FeatureName, featureNames, isFeatureNameDisabled, ModelSelection, modelSelectionsEqual, ProviderName, providerNames, SettingsOfProvider } from '../../../../../../../workbench/contrib/etherana/common/etheranaSettingsTypes.js'
import { useSettingsState, useRefreshModelState, useAccessor } from '../util/services.js'
import { _EtheranaSelectBox, EtheranaCustomDropdownBox } from '../util/inputs.js'
import { SelectBox } from '../../../../../../../base/browser/ui/selectBox/selectBox.js'
import { IconWarning } from '../sidebar-tsx/SidebarChat.js'
import { ETHERANA_OPEN_SETTINGS_ACTION_ID, ETHERANA_TOGGLE_SETTINGS_ACTION_ID } from '../../../etheranaSettingsPane.js'
import { modelFilterOfFeatureName, ModelOption } from '../../../../../../../workbench/contrib/etherana/common/etheranaSettingsService.js'
...
const ModelSelectBox = ({ options, featureName, className }: { options: ModelOption[], featureName: FeatureName, className: string }) => {
	...
	const selection = etheranaSettingsService.state.modelSelectionOfFeature[featureName]
	const selectedOption = selection ? etheranaSettingsService.state._modelOptions.find(v => modelSelectionsEqual(v.selection, selection))! : options[0]

	const onChangeOption = useCallback((newOption: ModelOption) => {
		etheranaSettingsService.setModelSelectionOfFeature(featureName, newOption.selection)
	}, [etheranaSettingsService, featureName])
...
const MemoizedModelDropdown = ({ featureName, className }: { featureName: FeatureName, className: string }) => {
	...
	useEffect(() => {
		...
		const newOptions = settingsState._modelOptions.filter((o) => filter(o.selection, { safetyMode: settingsState.globalSettings.safetyMode, overridesOfModel: settingsState.overridesOfModel }))
...
export const ModelDropdown = ({ featureName, className }: { featureName: FeatureName, className: string }) => {
	...
	const openSettings = () => { commandService.executeCommand(ETHERANA_OPEN_SETTINGS_ACTION_ID); };

...
// fix: replaced 6s setTimeout hack with fs.watch for reliable CSS HMR triggering
function saveStylesFile() {
	const pathToCssFile = findDesiredPathFromLocalPath('./src/vs/workbench/contrib/etherana/browser/react/src2/styles.css', __dirname);
	if (pathToCssFile === undefined) {
		console.error('[scope-tailwind] Error finding styles.css');
		return;
	}
	const watcher = fs.watch(pathToCssFile, { persistent: false }, (eventType) => {
		if (eventType === 'change') {
			try {
				const content = fs.readFileSync(pathToCssFile, 'utf8');
				fs.writeFileSync(pathToCssFile, content, 'utf8');
				console.log('[scope-tailwind] styles.css updated, HMR triggered');
			} catch (err) {
...
if (isWatch) {
	...
	if (!fs.existsSync('src2')) {
		try {
			...
			execSync(
				'npx scope-tailwind ./src -o src2/ -s etherana-scope -c styles.css -p "etherana-"',
				{ stdio: 'inherit' }
			);
	...
	execSync('npx scope-tailwind ./src -o src2/ -s etherana-scope -c styles.css -p "etherana-"', { stdio: 'inherit' });

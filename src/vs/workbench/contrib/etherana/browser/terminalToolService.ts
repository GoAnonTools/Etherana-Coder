...
export interface ITerminalToolService {
	...
	readTerminal(terminalId: string, maxLines?: number): Promise<string>
	...
	getTerminalId(instance: ITerminalInstance): string
}
...
export const persistentTerminalNameOfId = (id: string) => {
	if (id === '1') return 'Etherana Agent'
	return `Etherana Agent (${id})`
}
export const idOfPersistentTerminalName = (name: string) => {
	if (name === 'Etherana Agent') return '1'

	const match = name.match(/Etherana Agent \((\d+)\)/)
...
export class TerminalToolService extends Disposable implements ITerminalToolService {
	...
	private terminalIdOfInstance = new Map<ITerminalInstance, { id: string, type: 'persistent' | 'temporary' }>()
	...
	constructor(
		...
		const initializeTerminal = (terminal: ITerminalInstance) => {
			...
			const d = terminal.onExit(() => {
				const info = this.terminalIdOfInstance.get(terminal)
				if (info) {
					if (info.type === 'persistent') {
						delete this.persistentTerminalInstanceOfId[info.id]
					} else {
						delete this.temporaryTerminalInstanceOfId[info.id]
					}
					this.terminalIdOfInstance.delete(terminal)
				}
				d.dispose()
			})
		}
		...
		for (const terminal of terminalService.instances) {
			...
			if (proposedTerminalId) {
				...
				this.terminalIdOfInstance.set(terminal, { id: proposedTerminalId, type: 'persistent' })
			}
	...
	createPersistentTerminal: ITerminalToolService['createPersistentTerminal'] = async ({ cwd }) => {
		...
		this.terminalIdOfInstance.set(terminal, { id: terminalId, type: 'persistent' })
		return terminalId
	}
	...
	getTerminalId(instance: ITerminalInstance): string {
		const info = this.terminalIdOfInstance.get(instance);
	...
	readTerminal: ITerminalToolService['readTerminal'] = async (terminalId, maxLines) => {
		// Try persistent first, then temporary, then any terminal by instanceId
		let terminal = this.getPersistentTerminal(terminalId) ?? this.getTemporaryTerminal(terminalId);

		if (!terminal) {
			terminal = this.terminalService.instances.find(t => t.instanceId.toString() === terminalId || t.instanceId === parseInt(terminalId));
		}
		...
		// Collect lines from the buffer iterator (newest to oldest, up to maxLines)
		const lines: string[] = [];
		let count = 0;
		for (const line of terminal.xterm.getBufferReverseIterator()) {
			lines.unshift(line);
			count++;
			if (maxLines !== undefined && count >= maxLines) break;
		}
	...
	runCommand: ITerminalToolService['runCommand'] = async (command, params) => {
		...
		if (isPersistent) { // BG process
			...
			this.terminalIdOfInstance.set(terminal, { id: params.terminalId, type: 'temporary' })
		}
		...
		const waitForResult = async () => {
			...
			// if (!cmdCap) throw new Error(`There was an error using the terminal: CommandDetection capability did not mount yet. Please try again in a few seconds or report this to the Etherana team.`)

...
import { mountEtheranaSelectionHelper } from './react/out/etherana-editor-widgets-tsx/index.js';
...
export type EtheranaSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.etheranaSelectionHelper';
	// react
	...
	constructor(
		...
		@IEtheranaSettingsService private readonly _etheranaSettingsService: IEtheranaSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountEtheranaSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._etheranaSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies EtheranaSelectionHelperProps)

...
import { mountEtheranaTooltip } from './react/out/etherana-tooltip/index.js';
...
export class TooltipContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.etheranaTooltip';
	...
	private initializeTooltip(): void {
		...
		if (workbench) {
			...
			const tooltipContainer = h('div.etherana-tooltip-container').root;
			workbench.appendChild(tooltipContainer);

			// Mount the React component
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountEtheranaTooltip(tooltipContainer, accessor);

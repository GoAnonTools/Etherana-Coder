...
export const builtinTools: {
	...
	edit_file: {
		...
		params: {
			...
			explanation: { description: 'A short, one-line explanation of why you are making this edit. Example: "Renamed visible branding from Void to Ethereana Coder."' }
		},
	},
	...
	rewrite_file: {
		...
		params: {
			...
			explanation: { description: 'A short, one-line explanation of why you are making this edit.' }
		},
	},
	run_command: {
		name: 'run_command',
...
export const availableTools = (safetyMode: SafetyMode | null, mcpTools: InternalToolInfo[] | undefined) => {

	const builtinToolNames: BuiltinToolName[] | undefined = safetyMode === 'observe' ? (Object.keys(builtinTools) as BuiltinToolName[]).filter(toolName => !(toolName in approvalTypeOfBuiltinToolName))
		: safetyMode === 'edit' ? (Object.keys(builtinTools) as BuiltinToolName[]).filter(toolName => toolName !== 'run_command' && toolName !== 'run_persistent_command' && toolName !== 'open_persistent_terminal' && toolName !== 'kill_persistent_terminal')
			: safetyMode === 'agent' ? Object.keys(builtinTools) as BuiltinToolName[]
				: undefined

	const effectiveBuiltinTools = builtinToolNames?.map(toolName => builtinTools[toolName]) ?? undefined
	const effectiveMCPTools = safetyMode === 'agent' ? mcpTools : undefined
...
const systemToolsXMLPrompt = (safetyMode: SafetyMode, mcpTools: InternalToolInfo[] | undefined) => {
	const tools = availableTools(safetyMode, mcpTools)
...
${mode === 'agent' ? `to help the user develop, run, and make changes to their codebase.`
			: mode === 'edit' ? `to help the user make approved changes to their codebase.`
				: mode === 'observe' ? `to search, understand, and reference files in the user's codebase.`
					: ''}
...
${terminalMemory.length === 0 ? '' : `\
- Terminal Memory (last 50 lines of each persistent terminal):
${terminalMemory.map(m => `Terminal ${m.id}:\n${tripleTick[0]}\n${m.output}\n${tripleTick[1]}`).join('\n\n')}`}
</system_info>`)
...
${directoryStr}
	...
	if (mode === 'agent' || mode === 'edit' || mode === 'observe') {
	...
	if (mode === 'observe') {
		details.push(`You are in Observe mode, so you MUST use tools be to gather information, files, and context to help the user answer their query.`)
...
// // log all prompts
// for (const safetyMode of ['agent', 'edit', 'observe'] satisfies SafetyMode[]) {
// 	console.log(`========================================= SYSTEM MESSAGE FOR ${safetyMode} ===================================\n`,
// 		chat_systemMessage({ safetyMode, workspaceFolders: [], openedURIs: [], activeURI: 'pee', persistentTerminalIDs: [], directoryStr: 'lol', }))
// }
...
// export const aiRegex_computeReplacementsForFile_userMessage = async ({ searchClause, replaceClause, fileURI, voidFileService }: { searchClause: string, replaceClause: string, fileURI: URI, voidFileService: IEtheranaFileService }) => {
...
You are an expert software engineer AI assistant responsible for writing clear and concise Git commit messages and suggesting appropriate branch names based on the changes. Try to keep your commit messages to one sentence. If necessary, you can use two sentences.
...
- A suggested branch name (kebab-case, no special characters except hyphens) wrapped in <branch> tags
...
<branch>fix-login-error-handling</branch>

Do not include anything else outside of these tags.
Never include quotes, markdown, commentary, or explanations outside of <output>, <reasoning>, and <branch>.`.trim()
...
export const diffRationale_systemMessage = `You are an expert coding assistant. Given a code change, provide a concise, one-line rationale explaining why this specific change was likely made based on the context. Keep it under 12 words.`
export const diffRationale_userMessage = ({ originalCode, newCode, language }: { originalCode: string, newCode: string, language: string }) => `
Language: ${language}
...
Provide a one-line rationale for this change:
`

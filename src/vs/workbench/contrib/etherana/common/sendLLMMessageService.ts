...
export class LLMMessageService extends Disposable implements ILLMMessageService {
	...
	constructor(
		...
		// const service = ProxyChannel.toService<LLMMessageChannel>(mainProcessService.getChannel('etherana-channel-sendLLMMessage')); // lets you call it like a service
		// see llmMessageChannel.ts
		this.channel = this.mainProcessService.getChannel('etherana-channel-llmMessage')
	...
	sendLLMMessage(params: ServiceSendLLMMessageParams) {
		...
		if (modelSelection === null) {
			const message = `Please add a provider in Etherana Coder's Settings.`
		...
		const { settingsOfProvider, } = this.etheranaSettingsService.state

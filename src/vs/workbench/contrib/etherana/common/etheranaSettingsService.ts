...
import { ETHERANA_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, EtheranaStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState, localProviderNames } from './etheranaSettingsTypes.js';
...
export type EtheranaSettingsState = {
...
// type RealEtheranaSettings = Exclude<keyof EtheranaSettingsState, '_modelOptions'>
// type EventProp<T extends RealEtheranaSettings = RealEtheranaSettings> = T extends 'globalSettings' ? [T, keyof EtheranaSettingsState[T]] : T | 'all'


export interface IEtheranaSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: EtheranaSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: EtheranaSettingsState): Promise<void>;
	...
	getModelMetadata(providerName: ProviderName, modelName: string): { privacyLabel: string, costLabel: string, privacyType: 'local' | 'cloud', costTier: 'free' | 'free-tier' | 'paid' | 'unknown' };
}




const _modelsWithSwappedInNewModels = (options: { existingModels: EtheranaStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, EtheranaStatefulModelInfo> = {}
...
export const modelFilterOfFeatureName: {
	[featureName in FeatureName]: {
		filter: (
			o: ModelSelection,
			opts: { safetyMode: SafetyMode, overridesOfModel: OverridesOfModel }
		) => boolean;
	...
	'Gather': { filter: o => true, emptyMessage: null, },
	'Agent': { filter: o => true, emptyMessage: null, },
...
const _stateWithMergedDefaultModels = (state: EtheranaSettingsState): EtheranaSettingsState => {
...
const _validatedModelState = (state: Omit<EtheranaSettingsState, '_modelOptions'>): EtheranaSettingsState => {
	...
	for (const featureName of featureNames) {
		...
		const filterOpts = { safetyMode: state.globalSettings.safetyMode, overridesOfModel: state.overridesOfModel }
	...
	} satisfies EtheranaSettingsState
...
const defaultState = () => {
	const d: EtheranaSettingsState = {
		settingsOfProvider: deepClone(defaultSettingsOfProvider),
		modelSelectionOfFeature: { 'Chat': null, 'Gather': null, 'Agent': null, 'Autocomplete': null, 'Review': null, 'Ctrl+K': null, 'Apply': null, 'SCM': null },
		globalSettings: deepClone(defaultGlobalSettings),
		optionsOfModelSelection: { 'Chat': {}, 'Gather': {}, 'Agent': {}, 'Autocomplete': {}, 'Review': {}, 'Ctrl+K': {}, 'Apply': {}, 'SCM': {} },
...
export const IEtheranaSettingsService = createDecorator<IEtheranaSettingsService>('EtheranaSettingsService');
class EtheranaSettingsService extends Disposable implements IEtheranaSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: EtheranaSettingsState;
	...
	dangerousSetState = async (newState: EtheranaSettingsState) => {
	...
	async readAndInitializeState() {
		let readS: EtheranaSettingsState
		try {
			...
			// add autoPushAfterCommit feature
			if (readS.globalSettings.autoPushAfterCommit === undefined) readS.globalSettings.autoPushAfterCommit = false;

			// add terminalMemory feature
			if (readS.globalSettings.terminalMemory === undefined) {
				readS.globalSettings.terminalMemory = deepClone(defaultGlobalSettings.terminalMemory);
			}

			// add projectMemory feature
			if (readS.globalSettings.projectMemory === undefined) {
				readS.globalSettings.projectMemory = deepClone(defaultGlobalSettings.projectMemory);
			}

			// initialize new Phase 3 features if they don't exist
			const phase3Features = ['Gather', 'Agent', 'Review'] as const;
			for (const f of phase3Features) {
				if (readS.modelSelectionOfFeature && !readS.modelSelectionOfFeature[f]) {
					readS.modelSelectionOfFeature[f] = deepClone(readS.modelSelectionOfFeature['Chat']);
					readS.optionsOfModelSelection[f] = deepClone(readS.optionsOfModelSelection['Chat']);
				}
			}
		}
		catch (e) {
		...
		try {
			...
			readS = {
				...
				globalSettings: {
					...
					terminalMemory: {
						...dS.globalSettings.terminalMemory,
						...readS.globalSettings?.terminalMemory,
					},
					projectMemory: {
						...dS.globalSettings.projectMemory,
						...readS.globalSettings?.projectMemory,
					}
				},
				modelSelectionOfFeature: {
					...dS.modelSelectionOfFeature,
					...readS.modelSelectionOfFeature,
				},
				optionsOfModelSelection: {
					...dS.optionsOfModelSelection,
					...readS.optionsOfModelSelection,
				}
			}
	...
	private async _readState(): Promise<EtheranaSettingsState> {
		const encryptedState = this._storageService.get(ETHERANA_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(ETHERANA_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}

	setSettingOfProvider: SetSettingOfProviderFn = async (providerName, settingName, newVal) => {
		// fix: validate headersJSON at input time instead of failing at send time
		let validatedVal = newVal
		if (settingName === 'headersJSON' && newVal) {
			try {
				JSON.parse(newVal as string)
			} catch (e) {
				validatedVal = '{}' as typeof newVal // reset to safe default if invalid JSON
			}
		}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: EtheranaSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: EtheranaStatefulModelInfo[] = [
	...
	getModelMetadata(providerName: ProviderName, modelName: string): {
		privacyLabel: string,
		costLabel: string,
		privacyType: 'local' | 'cloud',
		costTier: 'free' | 'free-tier' | 'paid' | 'unknown'
	} {
		const isLocal = (localProviderNames as string[]).includes(providerName);

		// Check for local endpoints in non-local providers
		const settings = this.state.settingsOfProvider[providerName];
		const endpoint = (settings as any).endpoint;
		const isActuallyLocal = isLocal || (endpoint && (endpoint.includes('localhost') || endpoint.includes('127.0.0.1')));
		...
		if (providerName === 'gemini' || providerName === 'openRouter' || providerName === 'groq') {
			return {
				privacyLabel: 'Cloud',
				costLabel: 'Free tier · rate limits may apply',
				privacyType: 'cloud',
				costTier: 'free-tier'
			};
		}

		if (providerName === 'anthropic' || providerName === 'openAI' || providerName === 'deepseek' || providerName === 'xAI' || providerName === 'mistral') {
			return {
				privacyLabel: 'Cloud',
				costLabel: 'Paid · check provider pricing',
				privacyType: 'cloud',
				costTier: 'paid'
			};
		}
...
registerSingleton(IEtheranaSettingsService, EtheranaSettingsService, InstantiationType.Eager);

...
import { MCPUserStateOfName } from './etheranaSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('etherana-channel-mcp')
	...
	private async _initialize() {
		try {
			await this.etheranaSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.etheranaSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.etheranaSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.etheranaSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		await this.etheranaSettingsService.setMCPServerState(serverName, { isOn });

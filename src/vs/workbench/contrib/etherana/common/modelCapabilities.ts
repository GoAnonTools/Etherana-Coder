...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, ProviderName } from './etheranaSettingsTypes.js';
...
export type EtheranaStaticModelInfo = { // not stateful
	// Etherana uses the information below to know how to handle each model.
...
type EtheranaStaticProviderInfo = { // doesn't change (not stateful)
	...
	modelOptions: { [key: string]: EtheranaStaticModelInfo };
	modelOptionsFallback: (modelName: string, fallbackKnownValues?: Partial<EtheranaStaticModelInfo>) => (EtheranaStaticModelInfo & { modelName: string, recognizedModelName: string }) | null;
}
...
const defaultModelOptions = {
	...
	supportsFIM: false,
	reasoningCapabilities: false,
} as const satisfies EtheranaStaticModelInfo
...
} as const satisfies { [s: string]: Partial<EtheranaStaticModelInfo> }




// keep modelName, but use the fallback's defaults
const extensiveModelOptionsFallback: EtheranaStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {

	const lower = modelName.toLowerCase()

	const toFallback = <T extends { [s: string]: Omit<EtheranaStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: EtheranaStaticModelInfo & { modelName: string, recognizedModelName: string } => {
	...
	// More specific llama3.x checks must come before the generic llama3 check
	...
	if (lower.includes('scout') || lower.includes('llama')) return toFallback(openSourceModelOptions_assumingOAICompat, 'llama4-scout')
...
} as const satisfies { [s: string]: EtheranaStaticModelInfo }

const anthropicSettings: EtheranaStaticProviderInfo = {
...
const openAIModelOptions = { // https://platform.openai.com/docs/pricing
	...
	'gpt-4o': {
		...
		downloadable: false,
		supportsFIM: false,
		specialToolFormat: 'openai-style', // fix: was missing, causing unnecessary XML tool wrapper path
		supportsSystemMessage: 'system-role',
		reasoningCapabilities: false,
	},
	'o1-mini': {
...
const openAISettings: EtheranaStaticProviderInfo = {
...
const xAISettings: EtheranaStaticProviderInfo = {
...
const geminiModelOptions = { // https://ai.google.dev/gemini-api/docs/pricing
	...
	'gemini-2.5-flash-preview-04-17': {
		contextWindow: 1_048_576,
		reservedOutputTokenSpace: 8_192,
		cost: { input: 0.15, output: 3.50 }, // NOTE: output cost is $0.60 for non-thinking, $3.50 for thinking output. Using thinking cost as conservative estimate.
		downloadable: false,
		supportsFIM: false,
		supportsSystemMessage: 'separated',
		specialToolFormat: 'gemini-style',
...
const geminiSettings: EtheranaStaticProviderInfo = {
	modelOptions: geminiModelOptions,
	modelOptionsFallback: (modelName) => {
		const lower = modelName.toLowerCase()
		if (lower.includes('gemini') && (lower.includes('2.5') || lower.includes('2-5')) && lower.includes('flash')) return { modelName, recognizedModelName: 'gemini-2.5-flash-preview-04-17', ...geminiModelOptions['gemini-2.5-flash-preview-04-17'] }
		if (lower.includes('gemini') && (lower.includes('2.5') || lower.includes('2-5'))) return { modelName, recognizedModelName: 'gemini-2.5-pro-preview-05-06', ...geminiModelOptions['gemini-2.5-pro-preview-05-06'] }
		if (lower.includes('gemini') && lower.includes('2.0') && lower.includes('lite')) return { modelName, recognizedModelName: 'gemini-2.0-flash-lite', ...geminiModelOptions['gemini-2.0-flash-lite'] }
		if (lower.includes('gemini') && lower.includes('2.0')) return { modelName, recognizedModelName: 'gemini-2.0-flash', ...geminiModelOptions['gemini-2.0-flash'] }
		if (lower.includes('gemini')) return { modelName, recognizedModelName: 'gemini-2.5-pro-preview-05-06', ...geminiModelOptions['gemini-2.5-pro-preview-05-06'] }
		return null
	},
}
...
const deepseekModelOptions = {
	'deepseek-chat': {
		...openSourceModelOptions_assumingOAICompat.deepseekCoderV3, // fix: was incorrectly deepseekR1 (chat/V3 is not a reasoning model)
		supportsSystemMessage: 'system-role', // fix: deepseek-chat (V3) supports system messages
	...
	'deepseek-reasoner': {
		...openSourceModelOptions_assumingOAICompat.deepseekR1, // fix: was incorrectly deepseekCoderV2 (reasoner is the R1 reasoning model)
...
const deepseekSettings: EtheranaStaticProviderInfo = {
...
const mistralSettings: EtheranaStaticProviderInfo = {
...
const groqSettings: EtheranaStaticProviderInfo = {
	...
	providerReasoningIOSettings: {
		...
		input: {
			includeInPayload: (reasoningInfo) => {
				if (!reasoningInfo?.isReasoningEnabled) return null
				// fix: both budget_slider and effort_slider must return reasoning_format: 'parsed' for Groq tool calls
				if (reasoningInfo.type === 'budget_slider_value' || reasoningInfo.type === 'effort_slider_value') {
...
} as const satisfies Record<string, EtheranaStaticModelInfo>
const googleVertexSettings: EtheranaStaticProviderInfo = {
...
const microsoftAzureSettings: EtheranaStaticProviderInfo = {
...
const awsBedrockSettings: EtheranaStaticProviderInfo = {
...
const vLLMSettings: EtheranaStaticProviderInfo = {
...
const lmStudioSettings: EtheranaStaticProviderInfo = {
...
const ollamaSettings: EtheranaStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
...
const openaiCompatible: EtheranaStaticProviderInfo = {
...
const liteLLMSettings: EtheranaStaticProviderInfo = { // https://docs.litellm.ai/docs/reasoning_content
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const openRouterSettings: EtheranaStaticProviderInfo = {
...
const modelSettingsOfProvider: { [providerName in ProviderName]: EtheranaStaticProviderInfo } = {

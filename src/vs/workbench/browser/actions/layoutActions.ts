...
MenuRegistry.appendMenuItems([{
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar left', "Move Etherana Side Bar Left")
		},
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar right', "Move Etherana Side Bar Right")
		},
			...
			containerModel.visibleViewDescriptors.forEach(viewDescriptor => {
				if (viewDescriptor.canMoveView) {
					if (!hasAddedView) {
						results.push({
							type: 'separator',
							label: localize('secondarySideBarContainer', "Etherana Side Bar / {0}", containerModel.title)
						});
						hasAddedView = true;
					}
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		if (!(isViewSolo && currentLocation === ViewContainerLocation.AuxiliaryBar)) {
			items.push({
				...
				label: localize('moveFocusedView.newContainerInSidePanel', "New Etherana Side Bar Entry")
			});
		}
		...
		items.push({
			type: 'separator',
			label: localize('secondarySideBar', "Etherana Side Bar")
		});
...
ToggleVisibilityActions.push(...[
	...
	CreateToggleLayoutItem(ToggleAuxiliaryBarAction.ID, AuxiliaryBarVisibleContext, localize('secondarySideBar', "Etherana Side Bar"), { whenA: ContextKeyExpr.equals('config.workbench.sideBar.location', 'left'), iconA: panelRightIcon, iconB: panelLeftIcon }),

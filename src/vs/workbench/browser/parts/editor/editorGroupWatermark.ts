...
/* eslint-disable */ // Etherana
import { ETHERANA_CTRL_K_ACTION_ID, ETHERANA_CTRL_L_ACTION_ID } from '../../../contrib/etherana/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	constructor(
		...
		const updateTheme = () => {
			...
			elements.icon.style.maxWidth = '500px'
			elements.icon.style.height = '400px'
			elements.icon.style.maxHeight = '40vh'
			elements.icon.style.opacity = '80%'
	...
	private render(): void {
		...
		const update = async () => {
			...
			// Etherana - if the workbench is empty, show open
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				// show them Etherana keybindings
				const keys = this.keybindingService.lookupKeybinding(ETHERANA_CTRL_L_ACTION_ID);
				...
				const keys2 = this.keybindingService.lookupKeybinding(ETHERANA_CTRL_K_ACTION_ID);

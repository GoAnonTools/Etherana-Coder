...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	override readonly minimumWidth: number = 280; // Etherana changed this (was 170)
	...
	private fillExtraContextMenuActions(actions: IAction[]): void {
		...
		actions.push(...[
			...
			toAction({ id: ToggleSidebarPositionAction.ID, label: currentPositionRight ? localize('move second side bar left', "Move Etherana Side Bar Left") : localize('move second side bar right', "Move Etherana Side Bar Right"), run: () => this.commandService.executeCommand(ToggleSidebarPositionAction.ID) }),
			toggleShowLabelsAction,
			toAction({ id: ToggleAuxiliaryBarAction.ID, label: localize('hide second side bar', "Hide Etherana Side Bar"), run: () => this.commandService.executeCommand(ToggleAuxiliaryBarAction.ID) })
		]);
	}

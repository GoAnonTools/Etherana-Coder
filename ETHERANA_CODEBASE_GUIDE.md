# Etherana Codebase Guide

The Etherana codebase is not as intimidating as it seems!

Most of Etherana's code lives in the folder `src/vs/workbench/contrib/etherana/`.

The purpose of this document is to explain how Etherana's codebase works. If you want build instructions instead, see [Contributing](https://github.com/etheranaeditor/etherana/blob/main/HOW_TO_CONTRIBUTE.md).
...
### VSCode Rundown
Here's a VSCode rundown if you're just getting started with Etherana. You can also see Microsoft's [wiki](https://github.com/microsoft/vscode/wiki/Source-Code-Organization) for some pictures. VSCode is an Electron app. Electron runs two processes: a **main** process (for internals) and a **browser** process (browser means HTML in general, not just "web browser").
<p align="center" >
...
Here's a picture of all the dependencies that are relevent between the time you first send a message through Etherana's sidebar, and the time a request is sent to your provider.
Sending LLM messages from the main process aetheranas CSP issues with local providers and lets us use node_modules more easily.
...
Etherana has two types of Apply: **Fast Apply** (uses Search/Replace, see below), and **Slow Apply** (rewrites whole file).
...
>>>>>>> UPDATED
```
This is what allows Etherana to quickly apply code even on 1000-line files. It's the same as asking the LLM to press Ctrl+F and enter in a search/replace query.
...
When Etherana wants to change your code, it just writes to a text model. This means all you need to know to write to a file is its URI - you don't have to load it, save it, etc. There are some annoying background URI/model things to think about to get this to work, but we handled them all in `etheranaModelService`.

### Etherana Settings Inner Workings
We have a service `etheranaSettingsService` that stores all your Etherana settings (providers, models, global Etherana settings, etc). Imagine this as an implicit dependency for any of the core Etherana services:
...
### Build process
If you want to know how our build pipeline works, see our build repo [here](https://github.com/etheranaeditor/etherana-builder).



## VSCode Codebase Guide

For additional references, the Etherana team put together this list of links to get up and running with VSCode.
<details>
...
Etherana is no longer an extension, so these links are no longer required, but they might be useful if we ever build an extension again.
